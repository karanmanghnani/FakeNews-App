...
def main():
    ...
    workbook = xlsxwriter.Workbook('BD_results_corpus.xlsx')
    ...
    for k,v in corpus_fakebr.items():
        ...
        for id_doc in id_docs:
            ...
            if id_doc >= lower_limit and id_doc < upper_limit:
                ...
                stats = {}
                stats['label'] = k
                stats['emotion'] = round(n_emotions/len(words),3)*100
                stats['subjectivity'] = round(n_subj/len(words),3)*100
                stats['affectivity'] = round(temp_vad['total']/len(words) ,3)*100
                stats['polarity'] = round(doc_stats['positive_ratio'] + doc_stats['negative_ratio'],3)*100
                stats['bp'] =  round((doc_stats['perceptuality'] + doc_stats['relativity'] + doc_stats['cognitivity'] + doc_stats['personal_concerns'] + doc_stats['biological_processes'] + doc_stats['social_processes'] + doc_stats['negation_words_ratio'])/6 , 3)*100 
                ...
                worksheet.write(row, col, round(stats['emotion']))
                col+=1

                worksheet.write(row, col, round(stats['subjectivity']))
                col+=1
                ...
                worksheet.write(row, col, doc_stats['positive_ratio'])
                col+=1

                worksheet.write(row, col, doc_stats['negative_ratio'])
                col+=1

                worksheet.write(row, col, doc_stats['perceptuality'])
                col+=1

                worksheet.write(row, col, doc_stats['relativity'])
                col+=1

                worksheet.write(row, col, doc_stats['cognitivity'])
                col+=1

                worksheet.write(row, col, doc_stats['personal_concerns'])
                col+=1

                worksheet.write(row, col, doc_stats['biological_processes'])
                col+=1

                worksheet.write(row, col, doc_stats['social_processes'])
