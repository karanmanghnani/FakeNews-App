...
def get_emotions(text):
    ...
    for i in emotion_ratio.keys():
        if(total_value != 0):
            emotion_ratio[i] = round( emotion_ratio[i]/total_value , 2)

    #print("Total number of emotions is: " + str(total_value))
    #print(emotion_count)
    #print(emotion_ratio)
    return emotion_count,emotion_ratio
...
def HomePage():
    if request.method == 'POST': 
        if('url' in request.values ):
            ...
            emotion_count,emotion_ratio = get_emotions(article_text)
            return render_template('result.html', title='FakeNews',posts=posts, article_text=article_text, emotion_count=emotion_count, emotion_ratio=emotion_ratio)
        else:
