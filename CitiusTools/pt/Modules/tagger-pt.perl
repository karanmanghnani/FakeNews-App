...
open (MODEL, $abs_path."/model/train-pt") or die "O ficheiro não pode ser aberto: $!\n";
binmode MODEL,  ':utf8';
my @train = <MODEL>;
...
my $Border = "(Fp|<blank>)";
my @cat_open = ("NC", "VMI", "VMS", "VMG", "VMP", "VMN", "AQ");
    ...
    while ($i<=$#entry) {
       ...
       ####REGRAS MORFOLOGICAS
		 ...
		 my  $cat = "VMG";
             ...
             elsif  ($unk[$pos] && $Token[$pos] !~ /[áéóú]/ && $Token[$pos] =~ /[\w]+[aií]d[ao](s)?$/) { ##se a forma e desconhecida acabada em -ado/ido, VMP
		 my  $cat = "VMP";
                 $Tag{$pos}{$cat} = "VMP0000";
                 $Lema{$pos}{$cat} = $Lema[$pos] ;
                 $unk[$pos]=0;
	     }
             elsif  ($unk[$pos] && $Token[$pos] !~ /[áéíóú]/ && $Token[$pos] =~ /[\w]+(e[ss]+e([sn])?|era([sn])?|a[ss]+e([sn])?|ara([sn])?)$/) { ##se a forma e desconhecida acabada em -iese/iera, VMSI
		 my  $cat = "VMS";
                 $Tag{$pos}{$cat} = "VMSI000";
                 $Lema{$pos}{$cat} = $Lema[$pos] ;
                 $unk[$pos]=0;
	     }
            elsif  ($unk[$pos] &&  $Token[$pos] !~ /[áéíóú]/ && $Token[$pos] =~ /[\w]+ria([sn])?$/) { ##se a forma e desconhecida acabada em -ría, VMI
		 my  $cat = "VMI";
                 $Tag{$pos}{$cat} = "VMIC000";
                 $Lema{$pos}{$cat} = $Lema[$pos] ;
                 $unk[$pos]=0;
